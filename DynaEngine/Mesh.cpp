...
Mesh::Mesh(const wchar_t* full_path) : Resource(full_path)
{
	...
	if (!err.empty()) throw std::exception("Failed to create Mesh");

	if (!res) throw std::exception("Failed to create Mesh");

	if (shapes.size() > 1) throw std::exception("Failed to create Mesh");
